...
and a priori they could differ. The theorem as stated uses one letter for both,
...
Everything the theorem asserts is already present in that computation, one step
earlier than where the theorem states it. Transposition is a single identity
between two pairings, and reading that one identity at different arguments
yields both the agreement of the scalars and the transport of the
coreflections. The proof below is the same identity read at $(\alpha_2, x)$
instead of at $(\alpha_2, \alpha_1^\vee)$.

## The proof

The first claim is that
$\langle \alpha_2, \psi_{\ast} x\rangle = c \langle \alpha_1, x\rangle$
for every $x \in N_1$.
...
The second claim is that the two sides of the theorem agree.
...
## What the hypotheses actually do

Six of the seven fields of the isogeny structure go unused, because the proof
reads only `h.transpose`. `root_line`, `root_line_surjective`,
...
whatever its source. The isogeny hypothesis is what supplies such
a pair in practice, and it is why the statement is phrased this way, but the
argument never touches it.

**The statement omits the hypothesis $c \ne 0$, and is sound without it.** The reason
...
derivable, so omitting it is free and the $c = 0$ instance is unreachable.

Rationality is doing nothing here. The proof needs a commutative base ring and
the pairing normalization, and it ignores every other property of $\mathbb{Q}$.
Ngô works with $\mathbb{Q}$-vector spaces because the isogeny is only an
isomorphism after tensoring with $\mathbb{Q}$, so the coefficient field comes
from the source and this lemma never asks for it.
...
pass without proving anything. The shared definition file is imported, since it
holds a structure and no proofs.

## What it does not say

This solution lacks a companion `example` instantiating the hypotheses at
concrete data. A statement with contradictory hypotheses typechecks and is
vacuous, so the convention is to exhibit a witness. The derivation of $c \ne 0$
above rules out the one obvious way for the hypotheses to be unsatisfiable, but
that is not the same as exhibiting a root datum, an index pair, and a scalar
that satisfy all of them at once.

The theorem is one compatibility inside the combinatorial input to the
non-standard fundamental lemma. It is silent on orbital integrals, on the
fundamental lemma itself, and on the other conditions in Définition 1.12.1,
which the proof never reads.
