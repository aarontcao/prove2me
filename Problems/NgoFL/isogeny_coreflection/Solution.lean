...
/-!
# Solution: coreflections transport along a root datum isogeny

The statement is restated here rather than imported. Importing
`Upstream.NgoFL.isogeny_coreflection.Statement` would pull its `sorry` into
this file's axiom closure, which is the reduction path this integration
refuses.
-/
