...
/-!
# Solution: a real quadratic system is equivalent to a single quartic equation

The statement is restated here rather than imported. Importing
`Upstream.BSS.quadratic_system_equiv_single_quartic.Statement` would pull its
`sorry` into this file's axiom closure, which is the reduction path this
integration refuses.

The witness is the sum of squares. Squaring doubles the total degree, so a
family of quadratics produces a quartic. Over an ordered field a finite sum of
squares vanishes exactly when every summand does, which is the equivalence of
the zero sets.

An earlier version of this docstring attributed the construction to a numbered
proposition of Blum, Shub and Smale 1989. That locator could not be checked
from here, because the publisher blocks this address, so it is gone.
`explanation.md` carries the full reference and says which sentence of it was
read from where.
-/
...
vacuous on the empty side. The system `X₀ = 0`, `1 = 0` has no real solution, so
the quartic the theorem produces vanishes nowhere. -/
...
vacuous on the nonempty side. The system `X₀ = 0` has the single real solution
`0`, so the quartic vanishes at `0` and not at `1`. -/
example : ∃ f : MvPolynomial (Fin 1) ℝ, f.totalDegree ≤ 4 ∧
